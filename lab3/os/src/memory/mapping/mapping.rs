...
impl Mapping {
    ...
    pub fn new() -> MemoryResult<Mapping> {
        ...
        let pages = match FRAME_ALLOCATOR.lock().alloc() {
            Result::Ok(frame_tracker) => frame_tracker,
            Result::Err(err) => panic!("{}", err)
        };
        let root_table = PageTableTracker::new(pages);
    ...
    pub fn find_entry(&mut self, vpn: VirtualPageNumber) -> MemoryResult<&mut PageTableEntry> {
        ...
        for vpn_slice in &vpn.levels()[1..] {
            if entry.is_empty() {
                // 如果页表不存在，则需要分配一个新的页表
                let new_pages = match FRAME_ALLOCATOR.lock().alloc() {
                    Result::Ok(frame_tracker) => frame_tracker,
                    Result::Err(err) => panic!("{}", err)
                };
                let new_table = PageTableTracker::new(new_pages);
